...
This repository contains the backend for the inventory management system. It provides REST APIs for authentication, product and category management, employee and profile management, sales and restock operations, and file uploads.
...
From `Backend`, start the dev services (PostgreSQL, MailHog, RustFS, and SQL Studio):
...
Configuration is profile-based and lives under `Backend/src/main/resources`.

### Dev profile

Defaults are set in `Backend/src/main/resources/application-dev.properties`.
...
Production configuration uses environment variables set via Docker or a process manager. See `Backend/docker-compose.yaml` for the full list.
...
Endpoints are grouped under controllers in `Backend/src/main/java/com/firomsa/inventory/v1/controller`:
