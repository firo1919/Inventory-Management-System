...
export default function RestocksPage() {
  ...
  const fetchRestocks = async () => {
    try {
      setLoading(true);
      const endpoint = isAdmin ? "/api/v1/admin/restocks" : "/api/v1/employee/restocks";
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400">
            {isAdmin ? "Audit all inbound stock logs across the system" : "Log and view your product restock entries"}
          </p>
        </div>
        <button
      ...
      <div className="bg-white dark:bg-[#13131a] border border-slate-200 dark:border-white/5 rounded-2xl shadow-sm overflow-hidden">
                ...
                restocks.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-50/50 dark:hover:bg-[#1c1c24]/10 text-slate-600 dark:text-slate-300">
                    ...
                    <td className="p-4 text-xs text-slate-400">
                      {new Date(r.timestamp || r.restockDate).toLocaleString(undefined, {
                        month: "short",
                        day: "numeric",
                        year: "numeric",
                        hour: "2-digit",
        ...
        <div className="p-4 border-t border-slate-200 dark:border-white/5 flex items-center justify-between text-xs text-slate-400 bg-slate-50/50 dark:bg-[#0a0a0f]/50">
          <p>
            Showing {restocks.length} of {totalElements} transactions
          </p>
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#13131a] border border-slate-200 dark:border-white/5 rounded-2xl w-full max-w-sm p-6">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Record Product Restock</h3>
            <form onSubmit={handleAddRestock} className="space-y-4">
              ...
              {selectedProductDetails && (
                <div className="p-3 bg-indigo-500/5 border border-indigo-500/10 rounded-xl text-[11px] space-y-1 text-indigo-400">
                  ...
                  <p>• Cost Price: ${selectedProductDetails.costPrice?.toFixed(2)}</p>
                </div>
              )}

              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1">Quantity Restocked *</label>
                <input
                  type="number"
                  min={1}
                  required
        ...
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#13131a] border border-slate-200 dark:border-white/5 rounded-2xl w-full max-w-sm p-6 text-center">
            ...
            <p className="text-xs text-slate-400 mb-6 font-medium">
              Are you sure you want to delete restock transaction ID: <span className="font-mono text-indigo-400">{selectedRestock.id.substring(0, 8)}...</span>? This deletes the log only and will not roll back inventory quantities automatically.
            </p>
