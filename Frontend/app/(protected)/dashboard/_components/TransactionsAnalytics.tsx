...
export function TransactionsAnalytics({ salesCount, restocksCount }: TransactionsAnalyticsProps) {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        const res = await analyticsService.getTrends(granularity, startDate, endDate);
        if (ignore) return;
        ...
        setError(errorObj?.response?.data?.message || "Failed to load transaction analytics");
      } finally {
        if (!ignore) {
          setLoading(false);
        }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        <div>
          <h3 className="font-bold text-slate-900 dark:text-white text-base flex items-center gap-2">
            Transactions Analytics <TrendingUp className="w-4 h-4 text-indigo-500" />
          </h3>
      ...
      <div className="flex gap-6 text-sm mb-6 px-2">
        <div className="flex flex-col">
          <span className="text-slate-500 dark:text-slate-400 text-xs flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-emerald-500 block" /> Sales
          </span>
          <span className="font-semibold text-slate-800 dark:text-slate-100 mt-1">{salesCount} total</span>
        </div>
        <div className="flex flex-col">
          <span className="text-slate-500 dark:text-slate-400 text-xs flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-purple-500 block" /> Restocks
          </span>
          <span className="font-semibold text-slate-800 dark:text-slate-100 mt-1">{restocksCount} total</span>
        </div>
      </div>
      ...
      <div className="h-72 w-full relative">
        {loading ? (
          ...
          <div className="h-full w-full flex flex-col items-center justify-center gap-3 rounded-xl border border-rose-500/20 bg-rose-500/5 dark:bg-rose-500/10 p-6 text-center">
            ...
            <div>
              <h4 className="text-sm font-semibold text-slate-900 dark:text-white">Failed to Load Analytics</h4>
          ...
          <div className="h-full w-full flex flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-slate-200 dark:border-white/10 text-center p-6">
            ...
            <p className="text-sm font-medium text-slate-600 dark:text-slate-300">No Transaction Data Available</p>
            <p className="text-xs text-slate-400">No transactions recorded for the selected date range.</p>
          </div>
        ) : (
