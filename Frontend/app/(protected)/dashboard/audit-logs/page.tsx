...
export default function AuditLogsPage() {
  ...
  const handleExport = async (format: "csv" | "json") => {
    ...
    try {
      ...
      if (resourceTypeFilter) filterPayload.resourceType = resourceTypeFilter;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold dark:text-white">Audit Log Viewer</h1>
          <p className="text-xs text-slate-400">Track and analyze backend system events, database updates, and authentication logs</p>
        </div>
      ...
      {!statsLoading && stats && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="bg-white dark:bg-[#13131a] border border-slate-200 dark:border-white/5 rounded-2xl p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Total Actions</p>
              <h4 className="text-lg font-bold text-slate-800 dark:text-white mt-0.5">{stats.totalLogsCount || 0}</h4>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-[#13131a] border border-slate-200 dark:border-white/5 rounded-2xl p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Successful Events</p>
              <h4 className="text-lg font-bold text-slate-800 dark:text-white mt-0.5">{stats.successCount || 0}</h4>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-[#13131a] border border-slate-200 dark:border-white/5 rounded-2xl p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Failed Events</p>
              <h4 className="text-lg font-bold text-slate-800 dark:text-white mt-0.5">{stats.failureCount || 0}</h4>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-[#13131a] border border-slate-200 dark:border-white/5 rounded-2xl p-4 flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Error Rate</p>
              <h4 className="text-lg font-bold text-slate-800 dark:text-white mt-0.5">
                {stats.totalLogsCount > 0 ? `${((stats.failureCount / stats.totalLogsCount) * 100).toFixed(1)}%` : "0%"}
              </h4>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-[#13131a] border border-slate-200 dark:border-white/5 rounded-2xl p-5 shadow-sm">
        ...
        <form onSubmit={handleFilterSubmit} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          <div>
            <label className="block text-[10px] font-semibold text-slate-400 mb-1">Resource Type</label>
            <input
              type="text"
          ...
          <table className="w-full text-left text-sm border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
              {loading ? (
                ...
                logs.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-[#1c1c24]/10 text-slate-600 dark:text-slate-300">
                    ...
                    <td className="p-4 text-xs font-medium">{item.resourceType}</td>
                    <td className="p-4 font-mono text-xs">{item.username}</td>
                    <td className="p-4">
                      <span
                    ...
                    <td className="p-4 font-mono text-[10px] text-slate-400">
                      {item.correlationId ? item.correlationId.substring(0, 8) + "..." : "-"}
                    </td>
                    <td className="p-4">
      ...
      {isDetailModalOpen && selectedLog && (
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#13131a] border border-slate-200 dark:border-white/5 rounded-2xl w-full max-w-lg p-6">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Audit Log Metadata</h3>
            
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-xs">
                <div>
                  <span className="text-[10px] text-slate-400 uppercase tracking-wider block">Timestamp</span>
                  <span className="font-semibold">{new Date(selectedLog.timestamp).toLocaleString()}</span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 uppercase tracking-wider block">Correlation ID</span>
                  <span className="font-mono text-indigo-400">{selectedLog.correlationId || "None"}</span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 uppercase tracking-wider block">Action</span>
                  <span className="font-semibold text-slate-800 dark:text-white">{selectedLog.action}</span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 uppercase tracking-wider block">Resource Type</span>
                  <span className="font-semibold">{selectedLog.resourceType}</span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 uppercase tracking-wider block">User ID</span>
                  <span className="font-mono">{selectedLog.userId || "None"}</span>
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 uppercase tracking-wider block">Username</span>
              ...
              {selectedLog.errorMessage && (
                <div>
                  <span className="text-[10px] text-slate-400 uppercase tracking-wider block mb-1">Error Message</span>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#13131a] border border-slate-200 dark:border-white/5 rounded-2xl w-full max-w-sm p-6">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2 flex items-center gap-2 justify-center text-red-500">
              <AlertTriangle className="w-5 h-5 animate-pulse" /> Bulk Clear System Logs
            </h3>
            <p className="text-xs text-slate-400 text-center mb-4">Select the date range of logs to permanently delete from the database.</p>
            ...
            <form onSubmit={handleBulkDelete} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-white/5">
                ...
                <button
                  type="submit"
                  disabled={loading || !deleteStartDate || !deleteEndDate}
