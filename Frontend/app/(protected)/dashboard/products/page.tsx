...
export default function ProductsPage() {
    ...
    const handleImageUpload = async (file: File) => {
        ...
        try {
            ...
            const presignData = await profileService.getPresignedUrl(
                file.name,
                file.type,
            );
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    ...
                    <p className="text-xs text-slate-400">
                        Add, track, modify, and deactivate inventory items
                    </p>
                </div>
                {isAdmin && (
                    <button
                            ...
                            {loading ? (
                                Array.from({ length: 3 }).map((_, i) => (
                                    <tr key={i} className="animate-pulse">
                                        <td
                                            colSpan={7}
                                            className="p-4 h-16 bg-slate-50/10 dark:bg-[#0e0e13]/10"
                                        />
                                    </tr>
                                ))
                            ) : products.length === 0 ? (
                                ...
                                products.map((p) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <td className="py-4 px-5 align-middle">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div className="min-w-0 flex-1 flex flex-col justify-between py-0.5 h-16">
                                                        <h4 className="font-bold text-slate-900 dark:text-white text-sm tracking-tight leading-snug truncate max-w-xs">
                                                            {p.name}
                                                        </h4>
                                            ...
                                            <td className="py-4 px-5 font-mono text-sm font-bold text-indigo-400/90 align-middle">
                                                {p.sku}
                                            </td>
                                            <td className="py-4 px-5 align-middle">
                                                    ...
                                                    p.categoryIds.length > 0 ? (
                                                        p.categoryIds.map(
                                                            (cid: string) => (
                                                                <span
                                                                    key={cid}
                                                                    className="px-2 py-0.5 rounded-lg bg-slate-100 dark:bg-white/5 text-[10px] font-bold text-slate-500"
                                                                >
                                                                    {categoryMap[
                                                                        cid
                                                                    ] || cid}
