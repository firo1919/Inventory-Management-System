...
async function handleProxy(
    ...
    const accessToken =
        user?.accessToken || request.cookies.get("backend_access_token")?.value;
    const refreshToken =
        user?.refreshToken ||
    ...
    if (response.status === 401 && refreshToken) {
        ...
        try {
            ...
            if (refreshRes.ok) {
                ...
                console.warn(
                    "[Proxy] Token refresh failed with status",
                    refreshRes.status,
                );
            }
        } catch (refreshErr) {
    ...
    if (pathStr === "auth/logout") {
        finalResponse.cookies.set("backend_access_token", "", {
            maxAge: 0,
            path: "/",
        });
        finalResponse.cookies.set("backend_refresh_token", "", {
            maxAge: 0,
            path: "/",
        });
    }
