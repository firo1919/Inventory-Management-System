    ...
    <div className="w-full max-w-md bg-[#13131a]/80 backdrop-blur-xl border border-white/5 p-8 rounded-2xl shadow-2xl relative">
            ...
            {digits.map((digit, i) => (
              <input
                ...
                className={`w-12 h-14 text-center text-xl font-bold font-mono rounded-xl border transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500/70 focus:border-indigo-500 bg-[#0a0a0f]/60 text-white ${
                  digit ? "border-indigo-500/60 bg-indigo-500/10" : "border-white/10"
                }`}
      ...
      <div className="mt-8 text-center text-xs text-gray-500">
        {timer > 0 ? (
          <p>
            Resend OTP in{" "}
            <span className="text-indigo-400 font-semibold">{timer}s</span>
          </p>
        ) : (
          <button
            onClick={handleResend}
            disabled={resending || !email}
            className="text-indigo-400 hover:text-indigo-300 transition-colors font-medium flex items-center gap-1.5 mx-auto disabled:opacity-50 cursor-pointer"
          >
            {resending ? (
              <Loader size={3.5} className="flex-row" />
            ) : (
